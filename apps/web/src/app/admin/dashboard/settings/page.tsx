...
const SETTING_LABELS: Record<string, { label: string; type: 'number' | 'boolean' | 'string'; desc: string }> = {
  ...
  plaza_heartbeat_hide_minutes: { label: '策略广场隐藏超时 (分钟)', type: 'number', desc: '策略心跳中断超过此时间后，策略在广场中隐藏不展示。默认 5 分钟' },
  plaza_heartbeat_delete_minutes: { label: '策略广场删除超时 (分钟)', type: 'number', desc: '策略心跳中断超过此时间后，自动取消分享并从广场移除。默认 10080 分钟（7天）' },
