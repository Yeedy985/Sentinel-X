  ...
  return (
    <div className="p-5 xl:p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="p-6 rounded-2xl bg-white/[0.02] border border-slate-800/40">
          <h3 className="text-[15px] font-semibold text-slate-300 mb-4">今日详情</h3>
          <div className="space-y-2.5">
      ...
      <div className="rounded-2xl bg-white/[0.02] border border-slate-800/40 overflow-hidden">
        <div className="flex border-b border-slate-800/40 bg-white/[0.01]">
          <button
            ...
            className={`flex items-center gap-2 px-6 py-3.5 text-[15px] font-medium border-b-2 transition-all ${logTab === 'system' ? 'border-orange-400 text-orange-300' : 'border-transparent text-slate-500 hover:text-slate-300'}`}
          >
            <Cpu className="w-4.5 h-4.5" /> 扫描记录
            <span className="text-sm bg-slate-800/80 px-2.5 py-0.5 rounded-full">{systemTotal}</span>
          </button>
          <button
            onClick={() => setLogTab('calls')}
            className={`flex items-center gap-2 px-6 py-3.5 text-[15px] font-medium border-b-2 transition-all ${logTab === 'calls' ? 'border-cyan-400 text-cyan-300' : 'border-transparent text-slate-500 hover:text-slate-300'}`}
          >
            <Radio className="w-4.5 h-4.5" /> 调用记录
            <span className="text-sm bg-slate-800/80 px-2.5 py-0.5 rounded-full">{callTotal}</span>
          </button>
        </div>
        ...
        {logsLoading ? (
          ...
          <div>
            <div className="px-6 py-2.5 text-sm text-slate-500 border-b border-slate-800/30">
                ...
                <div key={s.id} className="px-6 py-4 flex items-center justify-between gap-4 hover:bg-white/[0.015] transition-colors">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2.5 flex-wrap">
                      <code className="text-sm font-mono text-slate-400">{s.briefingId}</code>
                      <StatusBadge status={s.status} />
                      {s.enableSearch && <span className="text-sm text-cyan-400 bg-cyan-500/10 px-2.5 py-0.5 rounded-md">搜索增强</span>}
                    </div>
                    ...
                    {s.errorMessage && <div className="text-sm text-red-400 mt-1">❌ {s.errorMessage}</div>}
                  </div>
                    ...
                    {s.status === 'COMPLETED' ? (
                      <>
                        <div className="flex items-center gap-1.5 justify-end">
                          ...
                          <span className="text-[15px] font-semibold text-amber-400">{s.tokenCostTotal.toLocaleString()} Token</span>
                        </div>
                      ...
                      <span className="text-[15px] text-amber-400">处理中</span>
                    )}
                  </div>
                </div>
              ))}
              ...
              {callLogs.map((r: any) => (
                <div key={r.id} className="px-6 py-4 flex items-center justify-between gap-4 hover:bg-white/[0.015] transition-colors">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2.5 flex-wrap">
                      {r.isCached ? (
                        <span className="text-sm font-medium px-2.5 py-0.5 rounded-md bg-amber-500/10 text-amber-400">缓存推送</span>
                      ) : (
                        <span className="text-sm font-medium px-2.5 py-0.5 rounded-md bg-emerald-500/10 text-emerald-400">真实调用</span>
                      )}
                      <span className="text-[15px] text-slate-300 font-medium">{r.userNickname || r.userEmail}</span>
                      <code className="text-sm font-mono text-slate-500">{r.briefingId}</code>
                    ...
                    {r.errorMessage && <div className="text-sm text-red-400 mt-1">❌ {r.errorMessage}</div>}
                  </div>
                  <div className="text-right shrink-0">
                    <div className="flex items-center gap-1.5 justify-end">
                      <Zap className="w-4 h-4 text-amber-400" />
                      <span className="text-[15px] font-semibold text-amber-400">-{r.tokenCost} Token</span>
                    </div>
                    <div className="text-sm text-slate-500 mt-1">
...
function StatCard({ icon: Icon, label, value, color, bg, sub }: { icon: any; label: string; value: any; color: string; bg: string; sub?: string }) {
  return (
    <div className="p-5 rounded-2xl bg-white/[0.02] border border-slate-800/40 hover:border-slate-700/50 transition-colors">
      ...
      {sub && <div className="text-sm text-slate-500 mt-1.5">{sub}</div>}
    </div>
  );
}
...
function DetailRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between items-center py-1.5">
      <span className="text-[15px] text-slate-400">{label}</span>
      <span className="text-[15px] font-medium text-slate-200">{value}</span>
    </div>
  );
}
...
function StatusBadge({ status }: { status: string }) {
  switch (status) {
    case 'COMPLETED':
      return <span className="text-sm px-2.5 py-0.5 rounded-md bg-emerald-500/10 text-emerald-400">完成</span>;
    case 'FAILED':
      return <span className="text-sm px-2.5 py-0.5 rounded-md bg-red-500/10 text-red-400">失败</span>;
    case 'PROCESSING':
      return <span className="text-sm px-2.5 py-0.5 rounded-md bg-blue-500/10 text-blue-400">处理中</span>;
    default:
      return <span className="text-sm px-2.5 py-0.5 rounded-md bg-slate-800 text-slate-400">排队中</span>;
  }
}
